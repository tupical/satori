...
use crate::{
    wrap_untrusted, AiOutput, AiProvider, AiRequest, Confidence, SensemakingError, SensingItem,
    SensingItemKind,
};
...
pub async fn sense_ai<P: AiProvider>(
    provider: &P,
    material: &str,
) -> Result<SensingItem, SensemakingError> {
    let req = AiRequest {
    ...
    Ok(SensingItem::new(result.kind, result.summary)
...
mod tests {
    ...
    async fn maps_structured_tool_call() {
        ...
        let item = sense_ai(&fake, "long material").await.unwrap();
