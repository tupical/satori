//! Concrete AI provider for satori-server: a minimal OpenAI Responses API
...
use satori::{AiError, AiOutput, AiProvider, AiRequest, ToolCall};
...
mod tests {
    ...
    fn build_body_minimal_and_with_tools() {
        ...
        let req = AiRequest {
            ...
            tools: vec![json!({"type": "function", "name": "do_thing"})],
    ...
    fn parse_outputs_message_and_function_call() {
        ...
        assert!(matches!(&out[1], AiOutput::ToolCall(tc) if tc.name == "do_thing"));
    }
